...
服务完整错误码前缀为 `200`（`BIZ` 命名空间、offset `0`）。共享领域错误范围为 `000-099`；当前使用 `DOMAIN_ENTITY_ID_INVALID(000)`、`DOMAIN_OBJECT_FIELD_REQUIRED(001)`、`DOMAIN_OBJECT_FIELD_INVALID(002)` 和 `DOMAIN_OBJECT_STATE_INVALID(003)`。认证账户、认证挑战、登录尝试和登录会话分别占用 `100-149`、`150-199`、`200-249`、`250-299`。角色、角色授权、权限、直接权限授权和渠道策略分别使用 `300-349`、`350-399`、`400-449`、`450-499`、`500-549`。应用错误范围为 `600-699`。
