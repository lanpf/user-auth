...
认证域会以领域事件表达账户、挑战、外部身份、登录尝试和登录会话的状态变化，并由应用层在本地事务内编排持久化和外部协作。原始 `DomainEvent` 只携带发生时间、事件类型和业务事实，不携带技术性的事件记录 ID；`DomainEventStore` 在持久化时为每条事件构造 `DomainEventEnvelope`，并通过 `domain_event` ID 空间分配趋势递增的 Long ID。领域对象因此只决定“是否发生事件以及事件内容”，不依赖 ID 生成端口或具体 EventStore 实现。

错误码与异常必须从所属子域的业务语义出发；已发布错误码不得改写、复用或重新分配。

服务完整错误码前缀为 `200`（`BIZ` 命名空间、offset `0`）。共享领域错误范围为 `000-099`；当前使用 `DOMAIN_ENTITY_ID_INVALID(000)`、`DOMAIN_OBJECT_FIELD_REQUIRED(002)`、`DOMAIN_OBJECT_FIELD_INVALID(003)` 和 `DOMAIN_OBJECT_STATE_INVALID(004)`，原事件 ID 校验码 `001` 随原始领域事件 ID 的移除而保留为空洞且不得复用。认证账户、认证挑战、登录尝试和登录会话分别占用 `100-149`、`150-199`、`200-249`、`250-299`。角色、角色授权、权限、直接权限授权和渠道策略分别使用 `300-349`、`350-399`、`400-449`、`450-499`、`500-549`。应用错误范围为 `600-699`。
