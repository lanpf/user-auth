# RestClientWechatMiniProgramApiClientLiveIT

## 测试目标

使用真实微信小程序 `appId`、`appSecret` 和 `wx.login` 产生的一次性 `loginCode`，调用微信
`jscode2session` 接口，验证 `RestClientWechatMiniProgramApiClient.exchangeLoginCode` 的真实网络连接、
...
mvn -pl user-auth-integration-tests -am verify \
  -Dit.test=RestClientWechatMiniProgramApiClientLiveIT
```
...
- `user-auth-integration-tests/target/failsafe-reports/com.cloud.userauth.infrastructure.external.wechat.miniprogram.client.RestClientWechatMiniProgramApiClientLiveIT.txt`
- `user-auth-integration-tests/target/failsafe-reports/TEST-com.cloud.userauth.infrastructure.external.wechat.miniprogram.client.RestClientWechatMiniProgramApiClientLiveIT.xml`
