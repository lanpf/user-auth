...
- **ARCH-MODULE-TECH-001** — Concrete persistence, scheduler, and message implementations live in `<service>-infrastructure-<kind>-<technology>` modules; each persistence implementation module owns its technology-specific DOs, DO conversion mappers, repositories, SQL/XML, and assembly.
...
- **ARCH-PROJECT-CONFIG-001** — Create a root-level `config/` directory beside service modules and prefer it for independently updateable application profiles, MyBatis SQL fragments and mapper XML, database schema or migration files, Dubbo XML, and similar configuration rather than packaging them in a module by default.
