...
- **INTERFACES-REST-CONTEXT-001** — After the gateway supplies trusted context, each controller business request is one mutable `@Valid` HTTP request extending `ClientRequest` and implements the composable `ChannelContext` and/or `AuthenticatedSessionContext` only when the endpoint requires those contexts. Do not split business fields into path or query parameters alongside a separate client-context argument. For endpoints without a business body, declare a concrete request type matching the required contexts and let the shared argument resolver build and validate it from protected headers.
